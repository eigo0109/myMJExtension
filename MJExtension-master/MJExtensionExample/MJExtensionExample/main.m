...
int main(int argc, const char * argv[])
{
    @autoreleasepool {
        ...
        execute(myTest, @"我自己的复杂model测试(float/int/NSInteger/bool/etc...)");
...
void myTest()
{
    ///实体（json或dict） to model （复杂）
    NSString *dictJsonStr = @"{\"str\":\"str\",\"null\":null,\"int1\":123,\"float1\":1.2,\"date\":\"1436096867\",\"bool1\":1,\"bool2\":true,\"sex\":1,\"key2\":\"theLongStrIsNotTaggedString\",\"iter1\":{\"iter2\":[{\"iter3\":\"iterStr1\"},{\"iter3\":\"iterStr2\"}]},\"innerModel\":{\"innerStr\":\"innerStr\",\"innerInt\":5,\"testModel\":{\"str\":\"__str\"}},\"selfModel\":{\"str\":\"selfStr\",\"null\":null,\"int1\":10},\"innerModelArray\":[{\"innerStr\":\"_arrstr1\"},{\"innerStr\":\"_arrstr2\"}]}";
    NSData *data = [dictJsonStr dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
    
    TestModel *model = [TestModel objectWithKeyValues:dictJsonStr];  //model from json
    TestModel *model2 = [TestModel objectWithKeyValues:dict]; //model from dict
    
    NSLog(@"DICT: %@", model.keyValues); //mode to  dict
    NSLog(@"DICT With Ignore: %@",[model keyValuesWithIgnoredKeys:@[@"bool2"]]); //mode to  dict with ignore
    NSLog(@"JSON String: %@",[model JSONString]); //model to json
    
    ///数组（json或Dict） to 数组 model
    NSString *arrJsonStr = @"[{\"name\":\"jack\",\"icon\":\"lufy.png\"},{\"name\":\"rose\",\"icon\":\"nami.png\"}]";
    
    NSData *data2 = [arrJsonStr dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *dictArray = [NSJSONSerialization JSONObjectWithData:data2 options:NSJSONReadingAllowFragments error:nil];
    
    NSArray *userArray = [User objectArrayWithKeyValuesArray:arrJsonStr];  //array（json） to array (model)
    NSArray *userArray2 = [User objectArrayWithKeyValuesArray:dictArray];  //array (dict) to array of model
    
    for (User *user in userArray) {
        NSLog(@"name=%@, icon=%@", user.name, user.icon);
    }
    
    NSArray *array = [User keyValuesArrayWithObjectArray:userArray]; //array (model) to array (dict)
    NSLog(@"%@",array);
}
